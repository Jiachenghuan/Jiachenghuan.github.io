...
const PrismaticBurst = ({
  ...
  const resolvedColors = [color0, color1, color2].filter(
    (value): value is string => typeof value === 'string' && value.trim().length > 0,
  );
  const gradientColors = resolvedColors.length > 0 ? resolvedColors : colors;
  ...
  useEffect(() => {
    ...
    if (Array.isArray(gradientColors) && gradientColors.length > 0) {
      const gl = renderer.gl;
      const capped = gradientColors.slice(0, 64);
      count = capped.length;
  ...
  }, [intensity, speed, animationType, gradientColors, distort, offset, rayCount]);
